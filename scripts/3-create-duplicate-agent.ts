...
async function createDuplicateAgent() {
  try {
    ...
    const configPath = path.resolve(process.cwd(), 'agent_config_download.json');
    if (!fs.existsSync(configPath)) {
      console.error('❌ No agent config found. Run: npx tsx scripts/1-download-config.ts');
      process.exit(1);
    }
    
    const currentConfig = JSON.parse(fs.readFileSync(configPath, 'utf-8'));
    console.log(`📋 Base config loaded: ${currentConfig.name} (${currentConfig.agent_id})`);
    ...
    if (!response || !response.ok) {
      console.log('⚠️ Agent creation via API not working. Manual creation required.');
      console.log('📋 Please manually create agent with this config and update the constants:');
      ...
      console.log('3. Copy the agent ID and update shared/constants/agentConfig.ts');
    ...
    const constantsPath = path.resolve(process.cwd(), 'shared/constants/agentConfig.ts');
    ...
    console.log(`   VITE_ELEVENLABS_ADDRESS_AGENT_TEST_ID="${newAgent.agent_id}"`);
