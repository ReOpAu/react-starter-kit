...
async function duplicateAgent() {
  try {
    ...
    const response = await fetch(`https://api.elevenlabs.io/v1/convai/agents/${agentId}/duplicate`, {
      method: 'POST',
      headers: {
    ...
    const verifyResponse = await fetch(`https://api.elevenlabs.io/v1/convai/agents/${newAgentId}`, {
    ...
    if (verifyResponse.ok) {
      ...
      console.log('⚠️ Could not verify new agent, but duplication reported success');
    }
    ...
    console.log('   npx tsx scripts/4-multi-agent-sync.ts --agent=ADDRESS_FINDER_TEST');
    console.log('3. Verify original agent unchanged:');
    console.log('   npx tsx scripts/4-multi-agent-sync.ts --agent=ADDRESS_FINDER --dry-run');
