...
async function syncConversationAgent(dryRun = false) {
	try {
				...
				function: {
					...
					parameters: {
						...
						properties: Object.fromEntries(
							Object.entries(toolDef.parametersSchema._def.shape || {}).map(
...
Parameters: ${JSON.stringify(toolDef.parametersSchema._def.shape || {}, null, 2)}`;
	...
	.join("\n\n")}`;
		...
		console.log(
			`📋 Updated: ${finalPrompt.length} characters, ${agentTools.length} tools`,
		);
