...
async function deleteAgent(agentId: string, confirm = false): Promise<void> {
	try {
		...
		if (getResponse.ok) {
			...
			console.log(
				`📅 Created: ${agentData.created_at_unix_secs ? new Date(agentData.created_at_unix_secs * 1000).toISOString() : "Unknown"}`,
			);
		} else {
			console.log(
				"⚠️ Could not fetch agent details, proceeding with deletion...",
			);
		}
		...
		if (!deleteResponse.ok) {
			...
			if (deleteResponse.status === 422) {
				console.error(
					"💡 Unprocessable Entity - Agent may have dependencies or restrictions",
				);
...
if (!agentId) {
	...
	console.log(
		"💡 Usage: npx tsx scripts/8-delete-agent.ts <agent_id> [--confirm]",
	);
...
console.log(
	`⚠️ Confirmation: ${confirm ? "YES - WILL DELETE" : "NO - DRY RUN"}`,
);
console.log("");
