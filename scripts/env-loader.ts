...
export function getElevenLabsConfig() {
  ...
  const agentId = env.ELEVENLABS_AGENT_ID || env.VITE_ELEVENLABS_ADDRESS_AGENT_ID;
  
  if (!apiKey) {
    throw new Error('Missing ELEVENLABS_API_KEY or VITE_ELEVENLABS_API_KEY in .env.local');
  }
  
  if (!agentId) {
    throw new Error('Missing ELEVENLABS_AGENT_ID or VITE_ELEVENLABS_ADDRESS_AGENT_ID in .env.local');
  }
