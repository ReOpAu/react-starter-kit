...
async function listAgents(
	...
	try {
		...
		result.agents.forEach((agent, index) => {
			...
			console.log(
				`   Created: ${new Date(agent.created_at_unix_secs * 1000).toISOString()}`,
			);
			console.log(
				`   Creator: ${agent.access_info.creator_name} (${agent.access_info.role})`,
			);
...
args.forEach((arg) => {
	if (arg.startsWith("--cursor=")) {
		...
		options.pageSize = Number.parseInt(arg.split("=")[1]);
