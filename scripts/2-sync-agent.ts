...
async function syncAgent(dryRun = false) {
  try {
    ...
    const apiTools = Object.entries(toolDefinitions).map(([name, def]) => {
      const schema = zodToJsonSchema(def.parametersSchema, { $refStrategy: 'none' });
    ...
    if (dryRun) {
      ...
      console.log('\n🔧 Tool names:', apiTools.map(t => t.function.name).join(', '));
    ...
    console.log('🎯 Target URL:', `https://api.elevenlabs.io/v1/convai/agents/${agentId}`);
