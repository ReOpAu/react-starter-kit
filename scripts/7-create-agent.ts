...
async function createAgent(
	name: string,
	basePrompt: string,
	voiceId = "HDA9tsk27wYi3uq0fPcK", // Default voice
): Promise<string> {
	try {
		...
		if (!response.ok) {
			...
			if (response.status === 422) {
				console.error(
					"💡 Unprocessable Entity - Check conversation configuration format",
				);
			}
...
if (!agentName) {
	...
	console.log(
		'💡 Usage: npx tsx scripts/7-create-agent.ts --name="My Agent" [--voice=voice_id] [--prompt=path/to/prompt.txt]',
	);
	process.exit(1);
}
...
if (customPrompt)
	console.log(`📄 Custom prompt: ${customPrompt.length} characters`);
console.log("");
