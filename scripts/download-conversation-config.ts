...
async function downloadConversationConfig() {
  try {
    ...
    console.log('🤖 Downloading config for CONVERSATION_ASSISTANT:', CONVERSATION_AGENT_ID);
    ...
    const currentPrompt = config.conversation_config?.agent?.prompt?.prompt || 'No prompt found';
    console.log('\n📄 Current prompt preview:');
    console.log('=' .repeat(50));
    console.log(currentPrompt.substring(0, 500) + (currentPrompt.length > 500 ? '...' : ''));
    console.log('=' .repeat(50));
