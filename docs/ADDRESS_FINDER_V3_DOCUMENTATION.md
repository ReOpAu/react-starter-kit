...
Whenever you move, add, or rename Convex backend actions (such as splitting actions into new files or changing their domain), you must run:

    npx convex dev

to regenerate the Convex API types. This ensures that the frontend can correctly import and use the new action paths (e.g., api.address.getPlaceSuggestions). If you see TypeScript errors about missing or incorrect API types, run this command and restart your dev server if needed.

## Convex API Usage Pattern: Address Actions

**Correct Usage:**
When calling Convex actions defined in subfolders (e.g., convex/address/getPlaceSuggestions.ts), always use the double-nested export as generated by Convex:
...
**Common Pitfall:**
Do NOT use string paths (e.g., `api["address/getPlaceSuggestions"]`) or single-nested properties (e.g., `api.address.getPlaceSuggestions`). These will not work with the generated API and will cause both linter and runtime errors such as:

- "Could not find public function for 'address:getPlaceSuggestions'"
- Type errors about missing properties or incorrect types

**How to Check:**
Always inspect the generated types in `convex/_generated/api.d.ts` to confirm the correct API structure for your project.
