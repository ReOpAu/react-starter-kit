...
function evaluateTestResult(
	...
	switch (expected) {
		case "SHOULD_PASS":
			if (isValid) {
				return { passed: true, score: "PASS", notes: "Correctly validated valid address" };
			} else {
				return { passed: false, score: "FAIL", notes: `Should have passed but failed: ${error}` };
			}

		case "SHOULD_FAIL":
			if (!isValid) {
				return { passed: true, score: "PASS", notes: `Correctly rejected invalid address: ${error}` };
			} else {
				return { passed: false, score: "FAIL", notes: "Should have failed but incorrectly validated" };
			}
...
async function runValidationTestsInternal(
	...
	if (categories && categories.length > 0) {
		...
		for (const category of categories) {
			if (category in testCategories) {
				const categoryTests = testCategories[category as keyof typeof testCategories];
	...
	for (const testCase of testCasesToRun) {
		...
		try {
			...
			const validationResult = {
				...
				formattedAddress: actualResult.success ? actualResult.suggestions[0]?.description : undefined,
				error: actualResult.success ? undefined : actualResult.error,
				placeId: actualResult.success ? actualResult.suggestions[0]?.placeId : undefined,
	...
	const categoryResults: Record<string, { total: number; passed: number; accuracy: number }> = {};
	for (const category of Object.keys(testCategories)) {
		...
		categoryResults[category] = {
			...
			accuracy: categoryTests.length > 0 ? (categoryPassed / categoryTests.length) * 100 : 0,
		};
	}
		...
		try {
			...
			if (args.expectedResult) {
				...
				const validationResult = {
					...
					formattedAddress: result.success ? result.suggestions[0]?.description : undefined,
