...
export const search = action({
  ...
  handler: async (_, { query }) => {
    ...
    if (!query || query.trim().length < 3) { // Require a minimum query length
    ...
    try {
      ...
      if (looksLikeFullAddress(query)) {
        const validatedSuggestions = await validateFullAddress(query, PLACES_API_KEY);
        if (validatedSuggestions.length > 0) {
          console.log(`[addressFinder.search] Found high-confidence match via Address Validation API for: "${query}"`);
      ...
      const suggestions = (data.predictions || [])
        ...
        .filter((p: GooglePlacesPrediction) => {
          ...
          return description.includes('australia') || /\b(vic|nsw|qld|wa|sa|tas|nt|act)\b/i.test(description);
        })
