...
export const getPlaceSuggestions = action({
	...
	handler: async (ctx, args) => {
		...
		try {
			...
			console.log(
				`[getPlaceSuggestions] Query: "${query}", Client Intent: ${clientIntent}, Final Intent: ${detectedIntent}`,
			);
			...
			if (isSingleWord && detectedIntent === "suburb") {
				console.log(
					"[getPlaceSuggestions] Single word, suburb intent. Trying suburb-first search.",
				);
				...
				if (suburbResult.success && suburbResult.suggestions.length > 0) {
					console.log(
						`[getPlaceSuggestions] Found ${suburbResult.suggestions.length} suburb results.`,
					);
					return suburbResult;
				}
				...
				console.log(
					"[getPlaceSuggestions] No suburb results found. Falling back to street search.",
				);
			...
			if (detectedIntent === "address" && !args.isAutocomplete) {
				console.log(
					"[getPlaceSuggestions] Using two-step validation for full address",
				);
			...
			if (detectedIntent === "address" && args.isAutocomplete) {
				console.log(
					"[getPlaceSuggestions] Autocomplete mode: Using Places API for address suggestions (no validation)",
				);
				return await getPlacesApiSuggestions(
					query,
					detectedIntent,
			...
			console.log(
				`[getPlaceSuggestions] Using Places API directly for intent: ${detectedIntent}`,
			);
			return await getPlacesApiSuggestions(
				query,
				detectedIntent,
				...
				const suburbMatch = addressData.predictions.find(
					(prediction: { types: string[]; description: string }) => {
						...
						const hasSpecificPlaceName =
							/\b(tunnel|bridge|station|mall|centre|center|park|reserve|oval|ground)\b/i.test(
								prediction.description,
							);
...
export const lookupSuburbEnhanced = action({
	...
	handler: async (ctx, args) => {
		...
		try {
			...
			console.log(
				"Enhanced Address-only API Response:",
				JSON.stringify(addressData, null, 2),
			);
...
export const lookupSuburbMultiple = action({
	...
	handler: async (ctx, args) => {
		...
		try {
			...
			console.log(
				"Multiple Address-only API Response:",
				JSON.stringify(addressData, null, 2),
			);
...
export const validateAddress = action({
	...
	handler: async (ctx, args) => {
		...
		try {
			...
			console.error(
				"[Address Validation] An unexpected error occurred:",
				error,
			);
			return {
				success: false as const,
...
async function validateAddressOnly(
	...
	try {
		...
		if (!addressComplete) {
			console.log(
				`[Address Validation Only] 🚨 REJECTING ADDRESS: "${address}" - Address is considered incomplete by the validation service.`,
			);
			return {
				isValid: false,
		...
		if (hasHouseNumber) {
			...
			if (result.address?.addressComponents) {
				for (const component of result.address.addressComponents) {
					if (component.confirmationLevel === "UNCONFIRMED_AND_SUSPICIOUS") {
						console.log(
							`[Address Validation Only] 🚨 REJECTING ADDRESS: "${address}" - Contains suspicious component: ${component.componentName?.text}`,
						);
						return {
							isValid: false,
		...
		const hasSignificantLocationChange = () => {
			...
			if (!inputHasSuburb && outputHasSuburb) {
				console.log(
					`[Address Validation Only] Input words: [${inputWords.join(", ")}]`,
				);
				console.log(
					`[Address Validation Only] Output words: [${outputWords.join(", ")}]`,
				);
				return true;
			}
		...
		if (hasSignificantLocationChange()) {
			console.log(
				`[Address Validation Only] 🚨 REJECTING ADDRESS: "${address}" - Google auto-completed to different location: "${formattedAddress}"`,
			);
			return {
				isValid: false,
				error:
		...
		console.log(
			`[Address Validation Only] Address is valid with granularity: ${validationGranularity}, hasHouseNumber: ${hasHouseNumber}`,
		);
...
> {
	console.log(
		`[Two-Step Validation] 🎯 Starting validation and enrichment for: "${query}"`,
	);

	// Step 1: Validate the address exists using Address Validation API
	console.log(
		"[Two-Step Validation] 🔍 Step 1: Calling validateAddressOnly...",
	);
	...
	if (!validationResult.isValid) {
		console.log(
			`[Two-Step Validation] Address validation failed: ${validationResult.error}`,
		);
		return {
			success: false,
	...
	console.log(
		`[Two-Step Validation] Address validated successfully. Formatted: "${validationResult.formattedAddress}"`,
	);
	console.log(
		"[Two-Step Validation] Now enriching with Places API to get suburb names...",
	);
	...
	if (placesResult.success) {
		console.log(
			`[Two-Step Validation] Successfully enriched with ${placesResult.suggestions.length} Places API results`,
		);
		...
		console.log(
			`[Two-Step Validation] Enhanced ${enhancedSuggestions.length} suggestions with suburb extraction`,
		);
		enhancedSuggestions.forEach((suggestion, index) => {
			if (suggestion.suburb) {
				console.log(
					`[Two-Step Validation] Suggestion ${index + 1}: "${suggestion.description}" -> Suburb: "${suggestion.suburb}"`,
				);
			}
		});
	...
	console.log(
		"[Two-Step Validation] Falling back to Address Validation API result only",
	);
	return await getAddressValidationSuggestions(
		query,
		maxResults,
		apiKey,
		"address",
...
> {
	try {
		console.log(
			`[Address Validation] Validating address: "${query}" with intent: ${actualIntent}`,
		);
		...
		if (result.address?.formattedAddress) {
			...
			if (result.verdict?.validationGranularity === "PREMISE")
				confidence += 0.15;
			else if (result.verdict?.validationGranularity === "SUB_PREMISE")
				confidence += 0.1;
...
> {
	try {
		...
		console.log(
			`[Places API] Single call with types: ${config.types} for intent: ${actualIntent}${sessionToken ? " with session token" : ""}`,
		);
