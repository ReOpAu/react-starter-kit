...
export const CLIENT_TOOLS = Object.keys(toolDefinitions) as Array<
	keyof typeof toolDefinitions
>;
...
export function getAllAgentIds(): string[] {
	return Object.values(ELEVENLABS_AGENTS).map((agent) => agent.id);
}

/**
 * Get agent configuration by transfer index (for agent-to-agent transfers)
 */
export function getAgentByTransferIndex(
	transferIndex: number,
): (typeof ELEVENLABS_AGENTS)[AgentKey] | null {
 ...
 */
export function recommendTransferAgent(
	...
	return (
		agents.find((agent) =>
			agent.specializations.includes(requiredSpecialization),
		) || null
	);
}
 ...
 */
export function validateAgentConfig(agentKey: AgentKey): {
	valid: boolean;
	errors: string[];
} {
	...
	const duplicateIndex = allAgents.filter(
		(a) => a.transferIndex === agent.transferIndex,
	);
