...
export function classifyLocationIntent(query: string): LocationIntent {
	...
	const isSpecialSuburb = specialCases.some((pattern) =>
	...
	if (isSpecialSuburb) {
		...
		const isCompleteSuburbName = completeSuburbNames.some((pattern) =>
			pattern.test(lowerQuery),
		);
		...
		const withoutSpecialPrefix = lowerQuery.replace(
			/^(st|mt|mount|port|glen|point)\s+/i,
			"",
		);
	...
	if (
		(hasPostcode || hasAustralianState || hasSuburbIndicators) &&
		!hasStreetType &&
		!hasRuralType
	) {
		return "suburb";
	...
	const isSuburbLikeText =
		ADDRESS_TEXT_PATTERNS.SUBURB_LIKE.test(lowerQuery) &&
