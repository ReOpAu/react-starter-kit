...
export default function AgentStatePanel({
	...
	const Section = ({
		...
		data,
		variant = "default",
	}: { title: string; data: Record<string, any>; variant?: string }) => (
		...
		<Card className={className}>
			<CardHeader>
				...
				<p className="text-xs text-gray-500">
					This panel shows exactly what the AI Agent is aware of right now.
				</p>
			</CardHeader>
			<CardContent className="space-y-4">
				...
				<Section
					...
					data={{
						...agentState.api,
						suggestions: `Array(${agentState.api.suggestions.length})`,
					}}
					variant="secondary"
				/>
				<Section
					title="Selection State"
					data={agentState.selection}
					variant="default"
				/>
				<ClientOnly>
					<Section
						...
						data={{
							...agentState.meta,
							lastUpdate: new Date(agentState.meta.lastUpdate).toLocaleString(),
						}}
						variant="default"
					/>
				</ClientOnly>
				{isExpanded && (
					<div className="pt-2">
						<Section
							title="Suggestions Detail"
							data={{ suggestions: agentState.api.suggestions }}
							variant="destructive"
						/>
					</div>
				)}
