...
export default function AgentDebugCopyPanel({
	agentState,
	history,
	className = "",
}: AgentDebugCopyPanelProps) {
	...
	const debugText = useMemo(() => {
		const lastHistory = history
			...
			.join("\n");
		const suggestions = agentState.api.suggestions
			.map(
		...
		<Card className={className}>
			<CardHeader>
				...
				<p className="text-xs text-gray-500">
					Copy this debug block and paste it into the Code editor or issue
					report for troubleshooting agent understanding issues.
				</p>
			</CardHeader>
			<CardContent>
				<textarea
