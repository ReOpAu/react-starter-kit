        ...
        <AccordionContent>
          <div className="space-y-4 pt-4 border-t">
            ...
            <div className="space-y-2 px-4">
              <h4 className="text-sm font-medium">Enhanced Address Autocomplete Test</h4>
              ...
              {suggestions.length > 0 && (
                <div className="bg-white p-2 rounded border">
                  <p className="text-xs text-muted-foreground mb-1">Results ({suggestions.length}):</p>
                  {suggestions.map((suggestion) => (
                    <div key={suggestion.placeId} className="text-xs p-1 border-b last:border-b-0">
                      <strong>{suggestion.address}</strong>
                      <span className="text-muted-foreground"> ({suggestion.addressType}, {suggestion.confidence})</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="flex flex-wrap justify-center gap-2 px-4">
              <Button
                ...
                onClick={async () => {
                  if (conversation.status === 'connected') {
                    await conversation.sendUserMessage('Please look up Yarraville');
                  } else {
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  resetSession();
                  console.log('[DEBUG] Spelling mode started, now processing input');
              ...
              <Button
                variant="outline"
                onClick={() => testAddressValidation('9999 Fake Street Nonexistent VIC')}
                disabled={isLoading}
                className="text-xs"
              >
              ...
              <Button
                variant="outline"
                onClick={() => testAddressValidation('123 Collins Street Melbourne VIC')}
                disabled={isLoading}
                className="text-xs"
              >
                🏢 Valid Address Test
              ...
              <Button
                variant="outline"
                onClick={() => testAddressValidation('1189 Clive Street West Footscray VIC')}
                disabled={isLoading}
                className="text-xs"
              >
                🔍 Test Clive Street
