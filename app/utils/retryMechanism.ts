  ...
  for (let attempt = 1; attempt <= finalConfig.maxAttempts; attempt++) {
    try {
      ...
      if (!finalConfig.retryCondition?.(error)) {
        console.warn(`[Retry] ${operationName} failed with non-retryable error:`, error);
        break;
      }
      ...
      console.warn(`[Retry] ${operationName} attempt ${attempt} failed, retrying in ${delay}ms:`, error);
