	...
	try {
		...
		if (typeof address === "string") {
			...
			if (Array.isArray(address.addressLines)) {
				addressPayload.addressLines = address.addressLines.filter(
		...
		if (!data.result) {
			return new Response(
				JSON.stringify({ error: "No validation result returned" }),
				{
					status: 400,
					headers: { "Content-Type": "application/json" },
				},
		...
		const enhancedResult = {
			...
			_validationSummary: {
				...
				hasUnconfirmedComponents:
					data.result.verdict?.hasUnconfirmedComponents || false,
				...
				unconfirmedComponentTypes:
					data.result.address?.unconfirmedComponentTypes || [],
				parsedComponents: data.result.address?.addressComponents?.reduce(
