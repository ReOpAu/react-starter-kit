...
export function EnhancedPlaceSuggestions({
	...
	const handleSearch = async () => {
		...
		await getPlaceSuggestions(query, currentIntent || undefined, {
			maxResults: 8,
		});
	};
	...
	const autoDetectedIntent = query
		? (classifyIntent(query) ?? undefined)
		: undefined;
	const currentIntent =
		(selectedIntent as LocationIntent) || autoDetectedIntent;
					...
					<div className="space-y-2">
						{isInputDisabled && (
							<div className="p-3 bg-orange-50 border border-orange-200 rounded-md">
								<p className="text-orange-700 text-sm font-medium">
									🎤 AI Mode Active - Manual search disabled. Use voice commands
						...
						<div className="flex gap-2">
							...
							<Button
								...
								disabled={isLoading || !query.trim() || isInputDisabled}
							>
						...
						<div className="text-xs text-muted-foreground bg-blue-50 p-2 rounded-md">
							💡 <strong>Smart Two-Step Validation:</strong> Full addresses
							(with numbers) are first validated for existence, then enriched
							with suburb names via Places API. Suburbs and streets use Places
							API directly.
						</div>
						...
						<div className="flex items-center gap-2 text-sm">
							...
							{query && currentIntent && (
								<Badge
									variant="outline"
									className={getIntentColor(currentIntent)}
								>
									{selectedIntent
										? `Override: ${currentIntent}`
										: `Auto: ${currentIntent}`}
								</Badge>
							)}
						</div>
				...
				<CardContent className="space-y-4">
					<div className="flex gap-2">
						...
						<Button
							...
							disabled={isLoading || !validationQuery.trim()}
						>
							...
							{validationResult.success && validationResult.result ? (
								<div className="space-y-3">
									...
									<div className="bg-gray-50 p-3 rounded-md">
										...
										<p className="text-sm">
											{validationResult.result.formattedAddress}
										</p>
									</div>
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
										<div className="bg-blue-50 p-3 rounded-md">
											...
											<p className="text-sm">
												{(() => {
													const lat =
														validationResult.result?.geocode?.latitude;
													const lng =
														validationResult.result?.geocode?.longitude;
													const latDisplay =
														typeof lat === "number" ? lat.toFixed(6) : "N/A";
													const lngDisplay =
														typeof lng === "number" ? lng.toFixed(6) : "N/A";
													return (
														<>
															Lat: {latDisplay}
															<br />
									...
									{Array.isArray(validationResult.result?.addressComponents) &&
										validationResult.result.addressComponents.length > 0 && (
											<div className="bg-gray-50 p-3 rounded-md">
												...
												<div className="flex flex-wrap gap-2">
													{validationResult.result.addressComponents.map(
														(comp, index) => (
															<Badge
											...
											<div className="mt-3">
												...
												<div className="space-y-1">
													{validationResult.issues.map((issue, index) => (
														<div
															key={`${issue.component}-${issue.severity}-${index}`}
						...
						<CardTitle className="flex items-center justify-between">
							...
							<div className="flex items-center gap-2">
								{detectedIntent && (
									<Badge
										variant="outline"
										className={getIntentColor(detectedIntent)}
									>
										Used: {detectedIntent}
									</Badge>
								)}
								{selectedIntent &&
									autoDetectedIntent &&
									selectedIntent !== autoDetectedIntent && (
										<Badge
											variant="outline"
										...
										<div className="flex-1">
											...
											{suggestion.suburb && (
												<div className="mb-2">
													<Badge
														variant="outline"
														className="bg-green-50 border-green-200 text-green-700 text-sm"
													>
					...
					<CardContent>
						<div className="space-y-3">
							...
							<div>
								...
								<Badge
									className={getResultTypeColor(selectedSuggestion.resultType)}
								>
