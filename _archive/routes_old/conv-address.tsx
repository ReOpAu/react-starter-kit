...
type CoreUIAction =
	...
	| {
			type: "SET_MODE";
			payload: { mode: CoreUIState["currentMode"]; action: string };
	  }
	| { type: "RESET_RESULTS" }
	| { type: "CLEAR_ALL" };
		...
		try {
			...
			if (typeof windowWithElevenLabs.setVariable === "function") {
				...
				const derivedUIState = {
					...
					hasResults:
						externalState.canonicalSuburb !== null ||
						coreState.selectedResult !== null,
				...
				windowWithElevenLabs.setVariable(
					"isRecording",
					derivedUIState.isRecording,
				);
				windowWithElevenLabs.setVariable(
					"isSpellingMode",
					derivedUIState.isSpellingMode,
				);
				windowWithElevenLabs.setVariable(
					"hasResults",
					derivedUIState.hasResults,
				);
				windowWithElevenLabs.setVariable(
					"hasMultipleResults",
					derivedUIState.hasMultipleResults,
				);
				windowWithElevenLabs.setVariable(
					"hasSelectedResult",
					derivedUIState.hasSelectedResult,
				);
				windowWithElevenLabs.setVariable(
					"currentMode",
					derivedUIState.currentMode,
				);

				// Result data
				windowWithElevenLabs.setVariable(
					"currentFoundResult",
					externalState.canonicalSuburb,
				);
				windowWithElevenLabs.setVariable(
					"currentSelectedResult",
					coreState.selectedResult?.canonicalSuburb || null,
				);
				windowWithElevenLabs.setVariable(
					"currentMultipleResults",
					coreState.multipleResults.map((r) => r.canonicalSuburb),
				);
				windowWithElevenLabs.setVariable(
					"currentSpellingCandidates",
					externalState.suggestions.map((s: any) => s.address),
				);

				console.log(
					"[UISync] Synced consolidated state to ElevenLabs:",
					derivedUIState,
				);
			}
		} catch (error) {
...
function isConfirmPlaceParams(
	...
	results: Array<{ type: string; place_id: string; label: string }>;
} {
	if (typeof params !== "object" || params === null) return false;
...
export default function ConvAddress() {
	...
	const enhancedAutocompleteAction = useAction(
		api.autocomplete.autocompleteAddresses,
	);
	...
	const { syncToElevenLabs } = useUIStateSync(coreState, {
		suggestions,
		canonicalSuburb,
	});
		...
		clientTools: {
			...
			AddressSearch: async (params: unknown) => {
				try {
					...
					if (typeof params === "string") {
						...
						address = String(
							paramObj.address ||
								...
								paramObj.input ||
								paramObj.value ||
								(dataObj && (dataObj.address || dataObj.suburb)) ||
								"",
						);
					} else {
						address = "";
					...
					if (
						...
						console.log(
							"[ClientTool] Invalid address parameter. Received:",
							params,
						);
						console.log(
							'[ClientTool] Expected format: { address: "suburb name" } or just "suburb name"',
						);

						// If we have existing multiple results, suggest the user clarify
						if (
							coreState.showMultipleResults &&
							coreState.multipleResults.length > 0
						) {
							const locations = coreState.multipleResults
								.map((r: SuburbResult) => r.canonicalSuburb)
								.join(", ");
					...
					if (looksLikeFullAddress) {
						console.log(
							"[ClientTool] Detected full address - will use Address Validation API first",
						);
					}
					...
					if (
						...
						console.log(
							"[ClientTool] Checking for clarification against existing results",
						);
						...
						for (const [intent, keywords] of Object.entries(intentKeywords)) {
							if (keywords.some((keyword) => lowerInput.includes(keyword))) {
								...
								matchedResult = coreState.multipleResults.find(
									(result: SuburbResult) => {
								...
								if (matchedResult) {
									console.log(
										`[ClientTool] Found intent-based match for ${intent}:`,
										matchedResult,
									);
									break;
								}
						...
						if (!matchedResult) {
							...
							for (const keyword of locationKeywords) {
								if (lowerInput.includes(keyword)) {
									...
									if (matchedResult) {
										console.log(
											`[ClientTool] Found location-based match for ${keyword}:`,
											matchedResult,
										);
										break;
									}
						...
						if (!matchedResult) {
							matchedResult = coreState.multipleResults.find(
								(result: SuburbResult) => {
									...
									return (
										canonical.includes(lowerInput) ||
										lowerInput.includes(canonical) ||
										canonical
											.split(" ")
											.some(
												(part: string) =>
													lowerInput.includes(part.toLowerCase()) &&
													part.length > 2,
											) ||
										canonical
											.split(",")
											.some((part: string) =>
												lowerInput.includes(part.trim().toLowerCase()),
											)
									);
								},
							);
						...
						if (matchedResult) {
							console.log(
								"[ClientTool] Found matching result for clarification:",
								matchedResult,
							);
							...
							const resultType = matchedResult.types?.includes("locality")
								? "suburb"
								: matchedResult.types?.includes("route")
									? "street"
									: matchedResult.types?.includes("street_address")
										? "address"
										: "location";
							...
							const responseMessage =
								`Perfect! ${emoji} I've confirmed you meant ${matchedResult.canonicalSuburb}. ` +
					...
					const addressPatterns = {
						...
						fullAddress:
							/^\d+[a-zA-Z]?\s+.+\s+(street|st|road|rd|avenue|ave|lane|ln|drive|dr|way|place|pl|court|ct|circuit|cct|crescent|cres|close|cl|boulevard|blvd|terrace|tce|parade|pde|grove|gr|walk|wlk|rise|highway|hwy|esplanade|esp)\s*,?\s*.+$/i,
						// Unit/apartment addresses
						unitAddress:
							/^(unit|apt|apartment|u|flat|f)\s*\d+[a-zA-Z]?\/?\s*\d+[a-zA-Z]?\s+.+\s+(street|st|road|rd|avenue|ave|lane|ln|drive|dr|way|place|pl|court|ct|circuit|cct|crescent|cres|close|cl|boulevard|blvd|terrace|tce|parade|pde|grove|gr|walk|wlk|rise|highway|hwy|esplanade|esp)\s*,?\s*.+$/i,
						// Simple number + street pattern
						simpleAddress: /^\d+[a-zA-Z]?\s+\w+/,
					};

					const isFullResidentialAddress =
						addressPatterns.fullAddress.test(address.trim()) ||
						addressPatterns.unitAddress.test(address.trim()) ||
						(addressPatterns.simpleAddress.test(address.trim()) &&
							address.includes(","));
					...
					console.log("[ClientTool] Address analysis:", {
						...
						simpleAddressMatch: addressPatterns.simpleAddress.test(
							address.trim(),
						),
					});
					...
					if (isFullResidentialAddress) {
						console.log(
							"[ClientTool] Detected full residential address - using Address Validation API for validation",
						);
						try {
							...
							console.log(
								"[ClientTool] Address Validation API results:",
								autocompleteResults,
							);
							...
							if (autocompleteResults && autocompleteResults.length > 0) {
								...
								const isAddressValidation =
									firstResult.matchType === "address_validation";
								console.log(
									"[ClientTool] Address Validation API used:",
									isAddressValidation,
								);
								...
								if (autocompleteResults.length === 1) {
									...
									responseMessage =
										`✅ Validated residential address: ${firstResult.address}. ` +
									...
									console.log(
										"[ClientTool] Single validated address result:",
										firstResult,
									);
								...
								console.log(
									"[ClientTool] Multiple address validation results - storing for selection:",
									autocompleteResults,
								);

								const compatibleResults = autocompleteResults.map(
									(result: any) => ({
								...
								const confirmPlaceResults = autocompleteResults.map(
									(result: any) => ({
								...
								try {
									...
									if (typeof windowWithElevenLabs.setVariable === "function") {
										...
										console.log(
											"[AddressSearch] Stored multiple validated results in ElevenLabs variables:",
											confirmPlaceResults,
										);
									}
								} catch (varError) {
									console.log(
										"[AddressSearch] ElevenLabs variables not available:",
										varError,
									);
								}
							...
							console.log(
								"[ClientTool] Address Validation API rejected the residential address",
							);
							...
							console.log(
								"[ClientTool] Address Validation API error:",
								validationError,
							);
					...
					console.log(
						"[ClientTool] Not a full residential address - using enhanced place suggestions for suburbs/streets...",
					);

					// First try the new Address Validation API system for all addresses
					try {
						console.log(
							"[ClientTool] Trying new autocomplete system with Address Validation API for:",
							address,
						);
						const autocompleteResults = await enhancedAutocompleteAction({
							partialInput: address,
						...
						console.log(
							"[ClientTool] Autocomplete results:",
							autocompleteResults,
						);
						...
						if (autocompleteResults && autocompleteResults.length > 0) {
							...
							if (autocompleteResults.length === 1) {
								...
								responseMessage =
									`Found ${isAddressValidation ? "validated address" : "address"}: ${firstResult.address}. ` +
									`${isAddressValidation ? "✅ Validated using Address Validation API" : `📍 Found via ${firstResult.matchType}`} ` +
									`with ${Math.round(firstResult.confidence * 100)}% confidence.`;

								console.log(
									"[ClientTool] Single result from new system:",
									firstResult,
								);
								...
								console.log(
									"[ClientTool] Multiple results from new system - storing for ConfirmPlace:",
									autocompleteResults,
								);
								...
								try {
									...
									if (typeof windowWithElevenLabs.setVariable === "function") {
										...
										console.log(
											"[AddressSearch] Stored multiple results in ElevenLabs variables:",
											confirmPlaceResults,
										);
									}
								} catch (varError) {
						...
						if (looksLikeFullAddress) {
							console.log(
								"[ClientTool] Full address rejected by Address Validation API - not falling back",
							);
						...
						console.log(
							"[ClientTool] Partial address - falling back to enhanced search for additional coverage...",
						);
					} catch (autocompleteError) {
						console.log(
							"[ClientTool] New autocomplete system failed:",
							autocompleteError,
						);
					...
					try {
						console.log(
							"[ClientTool] Calling enhanced place suggestions with:",
							address,
						);
						const enhancedResults = await searchPlaces(address);
						console.log(
							"[ClientTool] Enhanced place results:",
							enhancedResults,
						);

						if (
							enhancedResults.success &&
							enhancedResults.suggestions.length > 0
						) {
							...
							if (enhancedResults.suggestions.length === 1) {
								...
								console.log(
									"[AddressSearch] Single result found, getting full details via Convex:",
									suggestion,
								);

								try {
									// Use the Convex function to get full place details including coordinates
									const detailsResult = await lookupSuburbMultiple(
									...
									if (
										...
										"results" in detailsResult &&
										detailsResult.results.length > 0
									) {
										// Got full place details with coordinates
										const fullPlaceData = detailsResult.results[0];
										console.log(
											"[AddressSearch] Got full place details from Convex:",
											fullPlaceData,
										);
										...
										console.log(
											"[AddressSearch] Creating structured place data with coordinates:",
											structuredPlaceData,
										);

										// Store place data in shared state for ConfirmPlace to access
										dispatch({
											type: "SET_CURRENT_PLACE_DATA",
											payload: structuredPlaceData,
										});
										console.log(
											"[AddressSearch] Stored place data in shared state:",
											structuredPlaceData,
										);
										...
										try {
											...
											if (
												typeof windowWithElevenLabs.setVariable === "function"
											) {
												windowWithElevenLabs.setVariable(
													"placeSuggestions",
													structuredPlaceData,
												...
												console.log(
													"[AddressSearch] Stored data in ElevenLabs variables",
												);
											}
										} catch (varError) {
											console.log(
										...
										const geocode = fullPlaceData.geocode as
											| { lat: number; lng: number }
											| undefined;
										const coords =
											geocode &&
											...
											typeof geocode.lng === "number"
												...
												: "N/A";
										responseMessage =
											`Perfect! I found the ${suggestion.resultType} you're looking for: ${suggestion.structuredFormatting.mainText}. ` +
										...
										console.log(
											"[AddressSearch] Response message:",
											responseMessage,
										);
										...
										console.log(
											"[AddressSearch] Found single result with coordinates:",
											fullPlaceData,
										);
									...
									console.log(
										"[AddressSearch] Convex lookup failed, using autocomplete data",
									);
									const confidence = Math.round(suggestion.confidence * 100);
									...
									console.log(
										"[AddressSearch] Creating structured place data (no coordinates):",
										structuredPlaceData,
									);
									...
									console.log(
										"[AddressSearch] Stored fallback place data in shared state:",
										structuredPlaceData,
									);
									...
									console.log(
										"[AddressSearch] Response message (fallback):",
										responseMessage,
									);
									...
									// For single results, let ConfirmPlace handle the final confirmation
									console.log(
										"[AddressSearch] Found single result (fallback 1):",
										compatibleResult,
									);
									...
									console.log(
										"[AddressSearch] Convex lookup error:",
										convexError,
									);
									...
									console.log(
										"[AddressSearch] Creating structured place data (error fallback):",
										structuredPlaceData,
									);
									...
									console.log(
										"[AddressSearch] Stored error fallback place data in shared state:",
										structuredPlaceData,
									);
									...
									console.log(
										"[AddressSearch] Response message (error fallback):",
										responseMessage,
									);
									...
									// For single results, let ConfirmPlace handle the final confirmation
									console.log(
										"[AddressSearch] Found single result (fallback 2):",
										compatibleResult,
									);
								...
								const resultsByType = enhancedResults.suggestions.reduce(
									(acc: Record<string, any[]>, suggestion: any) => {
								...
								const typeDescriptions = Object.entries(resultsByType)
									.map(([type, suggestions]) => {
										...
										const examples = (suggestions as any[])
											.slice(0, 2)
											.map((s: any) => s.structuredFormatting.mainText)
											.join(", ");
								...
								responseMessage =
									`I found ${enhancedResults.suggestions.length} results for "${address}". ` +
								...
								const compatibleResults = enhancedResults.suggestions
									.slice(0, 8)
									.map((suggestion: any) => ({
										canonicalSuburb: suggestion.description,
										placeId: suggestion.placeId,
							...
							console.log(
								"[ClientTool] No enhanced results, falling back to suburb lookup",
							);
							const fallbackResult = await lookupSuburbMultiple(address, 5);

							if (
								fallbackResult.success &&
								"results" in fallbackResult &&
								...
								if (fallbackResult.results.length === 1) {
									...
									const fallbackPlaceData = {
										...
										secondaryText: place.canonicalSuburb
											.split(",")
											.slice(1)
											.join(",")
											.trim(),
									...
									console.log(
										"[AddressSearch] Creating fallback place data:",
										fallbackPlaceData,
									);
									...
									dispatch({
										type: "SET_CURRENT_PLACE_DATA",
										payload: fallbackPlaceData,
									});
									console.log(
										"[AddressSearch] Stored old lookup fallback place data in shared state:",
										fallbackPlaceData,
									);
									...
									console.log(
										"[AddressSearch] Old lookup fallback response message:",
										responseMessage,
									);
									...
									// For single results, let ConfirmPlace handle the final confirmation
									console.log(
										"[AddressSearch] Found single result (old lookup):",
										place,
									);
								} else {
									const locations = fallbackResult.results
										.map((r: SuburbResult) => r.canonicalSuburb)
										.join(", ");
						...
						console.error(
							"[ClientTool] Enhanced lookup failed:",
							enhancedError,
						);
			...
			ConfirmPlace: async (params: unknown) => {
				try {
					...
					if (results.length === 0) {
						console.log(
							"[ConfirmPlace] No results in params, checking ElevenLabs variables...",
						);
						try {
							const windowWithElevenLabs = window as typeof window & {
							...
							if (typeof windowWithElevenLabs.getVariable === "function") {
								const storedResults = windowWithElevenLabs.getVariable(
									"lastSearchResults",
								) as typeof results | undefined;
								if (
									storedResults &&
									...
									console.log(
										"[ConfirmPlace] Retrieved results from ElevenLabs variables:",
										results,
									);
								}
							}
							...
							console.log(
								"[ConfirmPlace] Could not access ElevenLabs variables:",
								varError,
							);
						}
					}
					...
					if (results.length === 0) {
						console.log(
							"[ConfirmPlace] No results available from params or variables",
						);
					...
					if (results.length === 1) {
						...
						console.log(
							"[ConfirmPlace] Single result provided, auto-confirming:",
							place,
						);
						...
						console.log(
							"[ConfirmPlace] Updated Selected Result with single result:",
							confirmedResult,
						);
					...
					console.log(
						"[ConfirmPlace] Multiple results provided, showing selection UI",
					);
					...
					console.log(
						"[ConfirmPlace] Displayed multiple results for user selection:",
						compatibleResults,
					);
			...
			GetUIState: async () => {
				const currentState = {
					...
					hasResults:
						canonicalSuburb !== null || coreState.selectedResult !== null,
					hasMultipleResults: coreState.showMultipleResults,
					hasSelectedResult: coreState.selectedResult !== null,
					resultCount: coreState.multipleResults.length,
					...
					multipleOptions: coreState.showMultipleResults
						? coreState.multipleResults.map((result: SuburbResult) => ({
	...
	const setupAudioAnalysis = useCallback(async () => {
		try {
			...
			const checkAudioLevel = () => {
				...
				if (
					isActive !== coreState.isVoiceActive &&
	...
	const handleSuburbLookup = useCallback(
		async (suburbInput: string, isSpellingAttempt = false) => {
			...
			searchResult =
				enhancedSearchResult.success &&
				"canonicalSuburb" in enhancedSearchResult
	...
	const handleTranscription = useCallback(
		async (text: string, source: string) => {
			...
			if (suggestions.length > 0) {
				console.log(
					"[Spelling] Processing spelled input with autocomplete:",
					text,
				);
			...
			if (result.success && "canonicalSuburb" in result) {
				...
				if (conversation.status === "connected") {
					const selectionSummary =
						`User has selected: "${suggestion.address}" from autocomplete suggestions during spelling mode. ` +
					...
					try {
						...
						console.log(
							"Failed to send autocomplete selection summary to agent:",
							error,
						);
					}
				}
				...
				try {
					...
					if (result && result.suggestions) {
						queryClient.setQueryData(
							["addressSearch", coreState.manualInput.trim()],
							result.suggestions,
						);
					...
					if (conversation.status === "connected") {
						try {
							await conversation.sendUserMessage(
								`Please look up this address: ${coreState.manualInput.trim()}`,
							);
							dispatch({ type: "SET_MANUAL_INPUT", payload: "" });
						} catch (agentError) {
	...
	const testMultipleResults = async (testInput: string) => {
		...
		try {
			...
			if (result.success && "results" in result) {
				...
				console.log(
					`[DEBUG] Found ${result.results.length} results:`,
					result.results.map((r: SuburbResult) => r.canonicalSuburb),
				);
			} else {
				console.log(
					"[DEBUG] No multiple results found:",
	...
	const testAddressValidation = async (fullAddress: string) => {
		...
		try {
			...
			if (result.length > 0) {
				...
				console.log(
					"[AddressValidation Test] Validated Address:",
					validatedAddress,
				);

				// Check if Address Validation API was used
				const isAddressValidation =
					validatedAddress.matchType === "address_validation";
				console.log(
					"[AddressValidation Test] Address Validation API used:",
					isAddressValidation,
				);
	...
	const handleResultSelection = async (result: SuburbResult) => {
		...
		if (conversation.status === "connected") {
			const selectionSummary =
				`User has selected: "${result.canonicalSuburb}" from multiple options. ` +
	...
	const handleEnhancedPlaceSelection = async (
		...
		if (conversation.status === "connected") {
			const selectionSummary =
				`User has selected: "${suggestion.description}" from enhanced place suggestions. ` +
			...
			try {
				...
				console.log(
					"Failed to send enhanced selection summary to agent:",
					error,
				);
			}
		}
	...
	const enhancedGeocode =
		enhancedResult &&
		(enhancedResult.geocode as { lat: number; lng: number } | undefined);
	const enhancedCoords =
		enhancedGeocode &&
		typeof enhancedGeocode.lat === "number" &&
		typeof enhancedGeocode.lng === "number"
			? `${enhancedGeocode.lat.toFixed(6)}, ${enhancedGeocode.lng.toFixed(6)}`
			: "N/A";
			...
			<div className="space-y-6">
				...
				<div className="text-center space-y-2">
					...
					<p className="text-muted-foreground">
						Speak or type an address, street name, or suburb to get validated
				...
				<DebugTools
					...
					setTestInput={(value: string) =>
						dispatch({ type: "SET_TEST_INPUT", payload: value })
					}
				...
				<Card>
					<CardHeader>
						<CardTitle className="flex items-center justify-between">
							...
							<div className="flex items-center gap-2">
								...
								{suggestions.length > 0 && (
									<Badge variant="default">Spelling Mode Active</Badge>
								)}
							</div>
						</CardTitle>
					</CardHeader>
					<CardContent className="space-y-4">
						...
						<div className="flex items-center justify-center gap-4">
							...
							{coreState.isRecording && (
								<VoiceIndicator isVoiceActive={coreState.isVoiceActive} />
							)}
						</div>
						...
						<form onSubmit={handleManualSubmit} className="flex gap-2">
							...
							<Button
								type="submit"
								disabled={isLoading || !coreState.manualInput.trim()}
							>
						...
						<Card className="bg-blue-50 border-blue-200">
							...
							<CardContent className="space-y-3">
								<p className="text-sm text-blue-700 mb-4">
									Found multiple places with similar names. Select the one
									you're looking for:
								</p>
								<div className="space-y-2">
									{coreState.multipleResults.map(
										(result: SuburbResult, index: number) => {
											const resultGeocode = result.geocode as
												| { lat: number; lng: number }
												| undefined;
											const resultCoords =
												resultGeocode &&
												typeof resultGeocode.lat === "number" &&
												typeof resultGeocode.lng === "number"
													? `${resultGeocode.lat.toFixed(4)}, ${resultGeocode.lng.toFixed(4)}`
													: "N/A";
											return (
												<Button
								...
								<Button
									...
									onClick={() =>
										dispatch({
											type: "SET_SHOW_MULTIPLE_RESULTS",
											payload: false,
										})
									}
						...
						<div className="text-xs font-mono space-y-2">
							<div>
								<p>
									...
									{coreState.selectedResult ? "HAS DATA" : "NULL"}
								</p>
								{coreState.selectedResult && (
									<>
										<p>
											<strong>Location:</strong>{" "}
											{coreState.selectedResult.canonicalSuburb}
										</p>
										<p>
											<strong>Place ID:</strong>{" "}
											{coreState.selectedResult.placeId}
										</p>
									</>
								)}
							</div>
							...
							<div className="border-t pt-2">
								<p>
									<strong>currentPlaceData:</strong>{" "}
									{coreState.currentPlaceData ? "HAS DATA" : "NULL"}
								</p>
								{coreState.currentPlaceData && (
									<>
										<p>
											<strong>Description:</strong>{" "}
											{coreState.currentPlaceData.description}
										</p>
										<p>
											<strong>Place ID:</strong>{" "}
											{coreState.currentPlaceData.placeId}
										</p>
									</>
								)}
							</div>
							...
							<div className="border-t pt-2">
								<p>
									<strong>ElevenLabs Variables:</strong>
								</p>
								<p>
									<strong>placeSuggestions:</strong>{" "}
									{!coreState.isClientMounted
										? "Loading..."
										: (() => {
												try {
													const windowWithElevenLabs =
														window as typeof window & {
															getVariable?: (name: string) => unknown;
														};
													const value =
														windowWithElevenLabs.getVariable?.(
															"placeSuggestions",
														);
								...
								<p>
									<strong>lastSearchResult:</strong>{" "}
									{!coreState.isClientMounted
										? "Loading..."
										: (() => {
												try {
				...
				{(() => {
					console.log(
						"[UI Render] selectedResult state:",
						coreState.selectedResult,
					);
					return null;
				})()}
					...
					<Card className="border-yellow-200 bg-yellow-50">
						...
						<CardContent className="space-y-4">
							{suggestions[0].address && (
								<div className="bg-white p-3 rounded border">
									...
									<p className="font-mono text-lg tracking-wider">
										{suggestions[0].address.toUpperCase()}
									</p>
								</div>
							)}
							...
							<div className="bg-gray-100 p-2 rounded text-xs">
								<p>
									<strong>Debug:</strong> isSpelling=
									{suggestions.length > 0 ? "true" : "false"}, suggestions=
									{suggestions.length}, currentSpelling="
									{suggestions[0].address}"
								</p>
							</div>
							...
							<div className="text-xs text-muted-foreground bg-white p-2 rounded border">
								<p>
									<strong>Tip:</strong> Speak each letter clearly with pauses:
									"Y... A... R... R... A... V... I... L... L... E"
								</p>
							</div>
						</CardContent>
					...
					<Card>
						...
						<CardContent>
							<div className="space-y-2 max-h-40 overflow-y-auto">
								{coreState.voiceTranscriptions.map(
									(transcription: VoiceTranscription) => (
										<div
											key={transcription.timestamp}
											className={cn(
												"flex items-center justify-between p-2 rounded",
												transcription.isSpelling
					...
					<Card className="bg-blue-50 border-blue-200">
						<CardHeader>
							<CardTitle className="text-lg flex items-center gap-2">
								...
								>
									{coreState.agentToolCalls.length}
								</Badge>
							</CardTitle>
						</CardHeader>
						<CardContent>
							<div className="space-y-3 max-h-40 overflow-y-auto">
								{coreState.agentToolCalls.map(
									(call: AgentToolCall, index: number) => (
										...
										>
											...
											<div className="ml-3 max-w-48">
												<Badge
													variant="outline"
													className="bg-blue-200 text-blue-800 text-xs truncate"
												>
													{call.result}
												</Badge>
											</div>
				...
				<Card>
					...
					<CardContent className="space-y-2">
						<p className="text-sm text-muted-foreground">
							1. <strong>Voice Input:</strong> Click "Start Voice Input" and
							speak an address, street name, or suburb clearly
						</p>
						<p className="text-sm text-muted-foreground">
							2. <strong>Manual Input:</strong> Type a full address, street
							name, or suburb in the text field and click "Search"
						</p>
						<p className="text-sm text-muted-foreground">
							3. <strong>Intent Detection:</strong> The system automatically
							detects whether you're looking for a suburb, street, or full
							address and optimizes results accordingly
						</p>
						<p className="text-sm text-muted-foreground">
							4. <strong>Enhanced Results:</strong> Results are organized by
							type (🏘️ Suburbs, 🛣️ Streets, 🏠 Addresses) with confidence scores
							and proper classification
						</p>
						<p className="text-sm text-muted-foreground">
							5. <strong>Smart Spelling:</strong> If an address isn't found, the
							system will ask you to spell it letter by letter with autocomplete
							suggestions
						</p>
						<p className="text-sm text-muted-foreground">
							6. The system queries Google Places API with intelligent filtering
							to return the most relevant results based on your intent
						</p>
						<p className="text-sm text-muted-foreground">
							7. All results are automatically saved to your search history with
			...
			<Card className="bg-gray-50 border-gray-200">
				...
				<CardContent>
					<div className="text-xs font-mono space-y-2">
						<div>
							<p>
								<strong>Current Mode:</strong> {coreState.currentMode}
							</p>
							<p>
								<strong>Last Action:</strong> {coreState.lastAction}
							</p>
							<p>
								<strong>Recording:</strong>{" "}
								{coreState.isRecording ? "YES" : "NO"}
							</p>
							<p>
								<strong>Voice Active:</strong>{" "}
								{coreState.isVoiceActive ? "YES" : "NO"}
							</p>
							<p>
								<strong>Spelling Mode:</strong>{" "}
								{suggestions.length > 0 ? "YES" : "NO"}
							</p>
							<p>
								<strong>Has Results:</strong>{" "}
								{canonicalSuburb || coreState.selectedResult ? "YES" : "NO"}
							</p>
							<p>
								<strong>Multiple Results:</strong>{" "}
								{coreState.showMultipleResults ? "YES" : "NO"}
							</p>
							<p>
								<strong>Selected Result:</strong>{" "}
								{coreState.selectedResult ? "YES" : "NO"}
							</p>
						</div>

						<div className="border-t pt-2">
							<p>
								<strong>Agent Sync Status:</strong>
							</p>
							<p className="text-green-600">
								✅ UI state continuously synced to ElevenLabs variables
							</p>
							<p className="text-blue-600">
								🔄 Agent can query UI state anytime via GetUIState tool
							</p>
							<p className="text-purple-600">
								🎯 Agent receives real-time updates on all UI changes
							</p>
						</div>
					</div>
				</CardContent>
